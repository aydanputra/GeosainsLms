...
function parseChatPayload(message: string): {
  ...
  for (const raw of lines) {
    ...
    if (lower.startsWith('produk:')) {
      const candidate = line.slice('Produk:'.length).trim();
      if (candidate) courseTitle = candidate;
      continue;
    }
...
function StudentInboxPageContent() {
  ...
  const isChatView = kind === 'messages';
  ...
  const clearMeta = useMemo(() => {
    ...
    const description =
      kind === 'messages'
        ? view === 'products'
          ? 'Ini akan menghapus riwayat pesan Produk & Layanan. Tindakan ini tidak bisa dibatalkan.'
          : view === 'admin'
  ...
  const unreadCount = useMemo(() => {
    if (isChatView) return filteredThreads.reduce((sum, t) => sum + (Number(t.unreadCount || 0) || 0), 0);
    return filteredItems.filter((n) => !n.read).length;
  }, [filteredItems, filteredThreads, isChatView]);
  ...
  }, [activeThreadId, filteredThreads, isChatView, isMobile, searchParams, threads]);
  ...
  const activeChatContext = useMemo(() => {
    ...
    const candidates = Array.isArray(dmMessages) && dmMessages.length > 0 ? dmMessages : null;
    if (candidates) {
      for (let i = candidates.length - 1; i >= 0; i -= 1) {
        const meta = parseChatPayload(String(candidates[i]?.body || ''));
        if (meta.courseTitle || meta.slug || meta.href || meta.commentId) {
          return meta;
        }
      }
    }
    if (activeThread) {
      const meta = parseChatPayload(String(activeThread.lastMessageText || ''));
      if (meta.courseTitle || meta.slug || meta.href || meta.commentId) return meta;
    }
    return null;
  }, [activeThread, dmMessages, isChatView, view]);
  ...
  const markOneRead = async (id: string) => {
    ...
    if (!res.ok) setItems((prev) => prev.map((n) => (n.id === id ? { ...n, read: false } : n)));
    else router.refresh();
  };
  ...
  const selectNotif = async (n: NotificationItem) => {
    setSelectedId(n.id);
  ...
  const tabClass = (active: boolean) =>
    active
      ? 'py-3 text-sm font-extrabold border-b-2 transition-colors whitespace-nowrap text-emerald-700 border-emerald-600'
  ...
  const pillClass = (active: boolean) =>
    twMerge(
      ...
      active ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'
    );
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-4">
              <div className="flex-1">
                <input
                  ...
                  className="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-900 placeholder:text-slate-400 shadow-sm outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                />
              </div>
                ...
                >
                  ...
                  {isLoadingThreads ? (
                    ...
                    <div className="flex-1 p-10 text-center text-slate-600 text-sm">
                      ...
                      <div className="mt-1 text-slate-600">Percakapan akan muncul saat ada direct message.</div>
                    </div>
                  ) : (
                        ...
                        <div className="px-5 pt-4">
                          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              {activeChatContext.slug ? (
                                <div className="text-xs text-slate-600 mt-1 truncate">{activeChatContext.slug}</div>
                              ) : null}
                            </div>
                            {activeChatContext.href ? (
                              <button
                                type="button"
                                onClick={() => router.push(activeChatContext.href!)}
                                className="h-9 px-3 rounded-xl bg-white border border-emerald-200 text-emerald-700 text-xs font-extrabold hover:bg-emerald-50 inline-flex items-center gap-2 shrink-0"
                              >
                        ...
                        {isLoadingDmMessages ? (
                          ...
                          <div className="space-y-3">
                            {visibleDmMessages.map((m) => {
                              ...
                              const meta = parseChatPayload(String(m.body || ''));
                ...
                >
                  <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between gap-3">
                    <div className="text-sm font-extrabold text-slate-900">
                      {view === 'enrollments' ? 'Pendaftaran' : view === 'purchases' ? 'Pembelian' : 'Umum'}
                    </div>
                    {unreadCount > 0 ? (
                      ...
                      {filteredItems.map((n) => {
                        ...
                        return (
                          <button
                            ...
                            className={twMerge('w-full text-left px-4 py-3 transition-colors', isActive ? 'bg-emerald-50' : 'hover:bg-slate-50')}
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
