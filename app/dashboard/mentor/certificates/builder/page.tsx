...
export default async function MentorCertificateBuilderPage({
  searchParams,
}: {
  ...
  if (courseId) {
    ...
    if (!course || course.deletedAt) redirect('/dashboard/mentor/certificates');
    ...
    if (user.role === 'MENTOR' && !isOwner) redirect('/dashboard/mentor/certificates');
